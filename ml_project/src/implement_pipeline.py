...
from src.constants import MODELS_DIR, CONFIG_DIR, DATA_DIR, DATA_RAW_DIR
...
def setup_parser(parser):
    ...
    parser.add_argument(
        ...
        required=True,
        default=False,
        help="name of model config file with path",
        metavar="FPATH",
    )
...
def model_pipeline(params: TrainingConfigParams):
    ...
    transformer.save(
        os.path.join(
            os.getcwd(), MODELS_DIR, params.model_folder, params.transformer_params.file
        )
    )
    ...
    with open(
            os.path.join(os.getcwd(), MODELS_DIR, params.model_folder, params.metric_file),
            "w",
    ) as metric_file:
        json.dump(metrics, metric_file)

    model.serialize_model(
        os.path.join(os.getcwd(), MODELS_DIR, params.model_folder, params.model_file)
    )
...
def model_creation_pipeline(config_path: str):
    ...
    setup_logging(
        params.logging_config,
        os.path.join(os.getcwd(), MODELS_DIR, params.model_folder, "train.log"),
    )
